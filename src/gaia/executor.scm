...
(define (rlm-repl-apropos env pattern-str)
  (let* ((sandbox (rlm-env-sandbox env))
         (m (sandbox-module sandbox))
         (syms (module-map (lambda (sym var) sym) m))
         (matches (filter (lambda (sym)
                            (string-contains-ci (symbol->string sym) pattern-str))
                          syms)))
    (if (null? matches)
        (list 'ok (format #f "No matches found for '~a' in sandbox." pattern-str))
        (list 'ok (string-join (map (lambda (sym)
                                      (let ((val (catch #t
                                                   (lambda () (module-ref m sym #f))
                                                   (lambda _ #f))))
                                        (format #f "  ~a = ~s" sym val)))
                                    (sort matches (lambda (a b)
                                                    (string<? (symbol->string a)
                                                              (symbol->string b)))))
                               "\n")))))

(define (rlm-repl-describe env sym)
  (let* ((sandbox (rlm-env-sandbox env))
         (m (sandbox-module sandbox))
         (val (catch #t
                (lambda () (module-ref m sym #f))
    ...
    (if (not val)
        ...
        (let* ((type-str (cond
                          ...
                          (else "value")))
               (doc (catch #t
                      (lambda () (procedure-documentation val))
                      (lambda _ #f)))
               (args (catch #t
                       (lambda () (and (procedure? val) (procedure-arguments val)))
                       (lambda _ #f)))
               (args-str (if args (format #f "\n  Arguments: ~a" args) ""))
               (doc-str (if doc (format #f "\n  Documentation: ~a" doc) "")))
          (list 'ok (format #f "Symbol: ~a\n  Type: ~a\n  Value: ~s~a~a"
                            sym type-str val args-str doc-str))))))

(define (rlm-repl-source env sym)
  (let* ((sandbox (rlm-env-sandbox env))
         (m (sandbox-module sandbox))
         (val (catch #t
                (lambda () (module-ref m sym #f))
    ...
    (if (not val)
        (list 'ok (format #f "Symbol '~a' is not defined in the sandbox." sym))
        (let ((src (procedure-source val)))
          (if src
              (list 'ok (format #f "Source of '~a':\n~s" sym src))
              (list 'ok (format #f "Source code for '~a' is not available (compiled code)." sym)))))))
...
to prevent blocking the Fibers scheduler, yielding control cooperatively."
  (let ((trimmed (string-trim-both code-string)))
    (cond
     ...
     ((or (string-prefix? ",describe" trimmed)
          (string-prefix? ",d" trimmed))
      (let* ((cmd-len (if (string-prefix? ",describe" trimmed) 9 2))
             (rest (string-trim-both (substring trimmed cmd-len)))
             (parts (string-split rest #\space))
             (sym-str (if (null? parts) "" (car parts))))
        (if (not (string-null? sym-str))
            (rlm-repl-describe env (string->symbol sym-str))
            (list 'ok "Usage: ,describe <symbol> - Show detailed description of a symbol.\n"))))
     ((or (string-prefix? ",apropos" trimmed)
          (string-prefix? ",a" trimmed))
      (let* ((cmd-len (if (string-prefix? ",apropos" trimmed) 8 2))
             (rest (string-trim-both (substring trimmed cmd-len))))
        (if (not (string-null? rest))
            (rlm-repl-apropos env rest)
            (list 'ok "Usage: ,apropos <pattern> - Search for sandbox symbols matching a pattern.\n"))))
     ((string-prefix? ",source" trimmed)
      (let* ((rest (string-trim-both (substring trimmed 7)))
             (parts (string-split rest #\space))
             (sym-str (if (null? parts) "" (car parts))))
        (if (not (string-null? sym-str))
            (rlm-repl-source env (string->symbol sym-str))
            (list 'ok "Usage: ,source <symbol> - Show source code of a procedure if available.\n"))))
