      ...
      (let loop ((env env)
        ...
        (let* ((raw-msg (get-message channel))
          ...
          (match msg
            ...
            ('interrupt
             ;; Just reset interrupted flag and loop (event already sent by reader fiber)
             (let ((core-mod (resolve-module '(gaia core) #:ensure #f)))
               (when core-mod
             ...
             (catch #t
               ...
               (lambda (key . args)
                 (if (eq? key 'user-interrupt)
                     (begin
                       (gaia-log "[SERVER] EVAL interrupted by user.")
                       (loop env history current-session-id))
            ...
            (('repl code)
             ...
             (catch #t
               ...
               (lambda (key . args)
                 (if (eq? key 'user-interrupt)
                     (begin
                       (gaia-log "[SERVER] REPL interrupted by user.")
                       (loop env history current-session-id))
            ...
            (('set-thinking state)
             ...
             (let ((state-bool (if (string=? state "on") #t #f)))
               ...
               (send-event client-socket `(final ,(string-append "Thinking mode set to: " state)))
               (loop env history current-session-id)))
            (('ask query)
            ...
            (('session new-id)
             (if (string-null? new-id)
                 (begin
                   (send-event client-socket `(final ,(string-append "Current session ID: " current-session-id)))
                   (loop env history current-session-id))
                 (begin
