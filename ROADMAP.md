...
- [/] **NCSI/J-space Integration Program** — Add a versioned NCSI contract, an optional RAI-hosted neural sidecar, a GAIA transport adapter and JSPACE processor, and evaluation against the existing textual projection. *(M0 is complete in both repositories. RAI implements the read-only sidecar and J-lens artifact path; GAIA implements the event adapter, epistemic invariants and JSPACE proposal conversion. Production UDS transport, bounded policy integration, live cross-project acceptance and M5 comparison remain open.)* Read-only neural observations precede any steering capability; signals remain observations and cannot directly confer `VERIFIED` or `ACCEPTED`. Detailed status is tracked only in the [canonical integration plan](docs/ncsi-jlens-integration.md).
