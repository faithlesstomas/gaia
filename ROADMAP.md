...
- [ ] **GCAS conversational assistant (P0)** — Plain input must create a bounded conversational Cognitive Process whose user turn, retrieved memories, Workspace broadcasts, assistant hypothesis, delivery evidence, and terminal answer are auditable COs/events. The LLM protocol history stays empty; its prompt is reconstructed from a bounded recent episode plus relevant structured memory. Assistant prose remains `HYPOTHESIS`/`UNVERIFIED`, while the independent completion Claim verifies only delivery. CLI and Emacs must resume the last logical session by default so continuity remains visible across client and server restarts.
