...
- [/] **NCSI/J-space Integration Program** — Add a versioned NCSI contract, an optional RAI-hosted neural sidecar, a GAIA transport adapter and JSPACE processor, and evaluation against the existing textual projection. *(GAIA implements the M0 contract, JSON fixtures, epistemic invariants, lifecycle audit, and JSPACE processor; the shared RAI conformance run and sidecar engine remain open.)*. Read-only neural observations precede any steering capability; signals remain observations and cannot directly confer `VERIFIED` or `ACCEPTED`. Detailed status is tracked only in the [canonical integration plan](docs/ncsi-jlens-integration.md).
