...
- [x] **Model-agnostic live evaluation matrix (P0)** — `make gcas-live-eval` runs the shared five-task verifier-backed corpus for exactly one explicitly approved model per invocation and rejects undeclared, multi-model, or over-4B runs before contacting an endpoint. It reports trajectories and model/task cells, first-pass and repair success, terminal rate, false completions, duplicate execution, calls, executions, tokens, latency, bounded interruption, and a machine-readable per-capability `READY/NOT_READY` verdict. Opt-in levels expose progress, heartbeat, GCAS/Workspace/CO state, processor routing, and a flushed JSONL trace without changing durable cognition. Reports from separate prompt/model runs remain directly comparable.
...
- [/] **Readiness gate (P1)** — Threshold enforcement is complete and fails closed below 10 repetitions, 100% terminal responses, zero false completions/duplicate executions, 80% verified success per capability, or the interruption bound. A repeated live baseline for one operator-approved ≤4B model is still required before any candidate capability becomes release-ready.
